...
async function checkSignal() {
  try {
    ...
    if (trade.active) {
      // TP/SL is now monitored in real-time via WebSocket (checkTpSl function)
      // This 10s cycle just logs status - no need to duplicate the check here
      console.log(`📊 Trade active (WS monitoring) | Price: ${livePrice || price} | TP: ${trade.tp} | SL: ${trade.sl}`);
      return;
    }
