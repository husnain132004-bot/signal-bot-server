...
// ║  Source:  Binance paxgusdt@kline_15m WebSocket (every trade tick)   ║
...
// ── Live candle state ─────────────────────────────────────────────────────
// Updated on every WebSocket kline_15m message (fires on every trade).
let liveKline = {
  openTime: 0,
  open:     0,
...
//  Computes NW (O(n²)), ATR, and RSI baseline — safe because called ≤1/15min
// ════════════════════════════════════════════════════════════════════════════
async function refreshHistoricalData() {
  try {
    ...
    console.log(
      ...
      ` | Last closed: ${new Date(hist.lastClosedOpenTime).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}`
    );
  } catch (err) {
...
function lockSetup(candleOpenTime, nwLower, atrLower, rsiAtLock) {
  ...
  console.log(
    `🔒 SETUP LOCKED | candle:${new Date(candleOpenTime).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}` +
...
function onLiveTick(liveClose, rsiLive) {
  ...
  switch (sm.state) {
    ...
    case S.LOCKED:
      ...
      if (liveSetupMet) {
        console.log(`🔄 REPLACED (LOCKED→LOCKED) | ${new Date(sm.setup.candleOpenTime).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})} → ${new Date(ot).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}`);
        lockSetup(ot, hist.nwLower, hist.atrLower, rsiLive);
      }
      break;
    ...
    case S.WAIT:
      ...
      if (sm.setup.confirmCandleOpenTime === null && ot !== sm.setup.candleOpenTime) {
        ...
        console.log(`⏳ Confirm candle started: ${new Date(ot).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}`);
      }
      ...
      if (sm.setup.confirmCandleOpenTime !== null &&
          ...
          liveSetupMet) {
        console.log(`🔄 REPLACED (WAITING→LOCKED) | → ${new Date(ot).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}`);
        lockSetup(ot, hist.nwLower, hist.atrLower, rsiLive);
      }
      break;
  }
  ...
  switch (sm.state) {
    ...
    case S.LOCKED:
      ...
      if (bullish) {
        ...
        console.log(`✅ SETUP CANDLE BULLISH → BUY`);
        await executeBuy(liveKline.close, sm.setup.atrLower, sm.setup.rsiAtLock);
      } else {
    ...
    case S.WAIT:
      ...
      if (bullish) {
        ...
        console.log(`✅ CONFIRM CANDLE BULLISH → BUY`);
        await executeBuy(liveKline.close, sm.setup.atrLower, sm.setup.rsiAtLock);
      } else {
...
// ════════════════════════════════════════════════════════════════════════════
//  SIGNAL WEBSOCKET — Binance kline_15m stream
...
function connectSignalWebSocket() {
  try {
    wsSig = new WebSocket('wss://stream.binance.com:9443/ws/paxgusdt@kline_15m');

    wsSig.on('open', () => {
      console.log('📡 Signal WS connected (kline_15m) — TRUE LIVE mode active');
    });
    ...
    wsSig.on('message', async (data) => {
      try {
        ...
        */
        ...
        if (k.x) {
          console.log(
            `🕯️  Candle closed ${new Date(k.t).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}` +
          ...
          if (hist.lastClosedOpenTime !== k.t) {
            console.log(
              `⚠️ REST lag detected: hist=${new Date(hist.lastClosedOpenTime).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})}` +
              ` vs WS-confirmed=${new Date(k.t).toLocaleTimeString('en-PK',{hour:'2-digit',minute:'2-digit'})} — retrying in 2s`
            );
            await new Promise(r => setTimeout(r, 2000));
            await refreshHistoricalData();

            if (hist.lastClosedOpenTime !== k.t) {
              console.log(`⚠️ REST still lagging after retry — will self-correct on next candle close or 15-min safety refresh`);
            } else {
...
// Safety net: re-sync history every 15 minutes in case WS close was missed
setInterval(refreshHistoricalData, 15 * 60 * 1000);

console.log('🤖 PAXG Signal Bot — TRUE LIVE mode | kline_15m WebSocket | State Machine ready');
