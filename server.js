...
async function checkSignal() {
  try {
    ...
    // ===== 2. ATR BANDS =====
    // TradingView indicator: "ATR 14 RMA 0.5 high low"
    //   → upper = RMA(High, 14) + ATR * 0.5
    //   → lower = RMA(Low,  14) − ATR * 0.5
    // "high low" = source: Highs for upper band, Lows for lower band
    const atr        = calcATR(closedHighs, closedLows, closedCloses, 14);
    const atrMidHigh = calcRMA(closedHighs, 14);   // RMA of candle HIGHS  → upper midline
    const atrMidLow  = calcRMA(closedLows,  14);   // RMA of candle LOWS   → lower midline
    const atrUpper   = atrMidHigh + atr * 0.5;     // upper band = RMA(high) + ATR*0.5
    const atrLower   = atrMidLow  - atr * 0.5;     // lower band = RMA(low)  − ATR*0.5  ← SL ref
    ...
    console.log(
      ...
      `📐 RMA(high, 14)    : ${atrMidHigh.toFixed(4)}\n` +
      `📐 RMA(low,  14)    : ${atrMidLow.toFixed(4)}\n` +
      `📐 ATR UPPER = ${atrMidHigh.toFixed(2)} + ${atr.toFixed(2)}×0.5 = ${atrUpper.toFixed(2)}\n` +
      `📐 ATR LOWER = ${atrMidLow.toFixed(2)} - ${atr.toFixed(2)}×0.5 = ${atrLower.toFixed(2)}\n` +
    ...
    indicators = {
      ...
      atrMidHigh: +atrMidHigh.toFixed(2),
      atrMidLow:  +atrMidLow.toFixed(2),
    ...
    console.log(
      ...
      ` | 📊 ATR Upper: ${fmt(atrUpper)} (midH:${fmt(atrMidHigh)}) | ATR Lower: ${fmt(atrLower)} (midL:${fmt(atrMidLow)})` +
