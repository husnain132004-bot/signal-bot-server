...
app.post('/delete-journal-entry', (req, res) => {
  const { index } = req.body;
  if (typeof index !== 'number' || index < 0 || index >= journal.length) {
    return res.status(400).json({ error: 'Invalid index' });
  }
  journal.splice(index, 1);
  saveJournal();
  console.log(`🗑 Journal entry ${index} deleted | ${journal.length} entries remaining`);
  res.json({ success: true, remaining: journal.length });
});
