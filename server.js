...
async function checkSignal() {
  try {
    ...
    // ATR Bands midline uses RMA (Wilder's Moving Average) — same as TradingView
    // RMA formula: rma[i] = (rma[i-1] * (period-1) + value[i]) / period
    ...
    const atrMid   = calcRMA(closedCloses, 14);
